...
const TEAM = {
  ...
  away: { fill: "#52525b", text: "#fee2e2", ring: "#f87171" },
} as const;
...
function arrowPath(a: PitchArrow) {
  const x1 = a.from.x, y1 = y(a.from.y);
  const x2 = a.to.x, y2 = y(a.to.y);
      ...
      {state.players.map((p) => {
        ...
        return (
          <g key={p.id}>
            ...
            <circle cx={p.x} cy={py} r={3.2} fill={c.fill} stroke="#09090b" strokeWidth={0.35} />
            <text
              x={p.x}
              y={py + 1.1}
              fontSize={2.9}
